...
class Tutorial : public TutorialBase
{
  public:
	void run()
	{
		...
		CHECK_ORO( cudaMalloc( reinterpret_cast<void**>( &sceneInput.instanceFrames ), sizeof( matrices ) ) );
		CHECK_ORO( cuMemcpyHtoD( reinterpret_cast<CUdeviceptr>( sceneInput.instanceFrames ), matrices, sizeof( matrices ) ) );
